...
if(isset($login_session) && $_SESSION['rolle'] >= 3) {
  ...
  if ($stmt = $mysqli->prepare("SELECT kurz, kontakt FROM posten order by kurz")) {
    ...
    while ($stmt->fetch()) {
      ...
      echo '<div class="ui-block-'.$bs.'"><a class="ui-btn ui-icon-comment ui-btn-icon-left" rel="noreferrer noopener" target="_blank" href="https://api.whatsapp.com/send?phone=49'.substr($zwei, 1).'" type="button">'.$eins.'</a></div>';
    }
  }
  echo '</div>';
